	...
	public UsersGUI() {
		...
		addButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				// 创建新建日记内部窗体
				final JInternalFrame internalFrame_Write = new JInternalFrame("Create a diary", false, true, false,
						false);
		...
		delButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (value == JFileChooser.APPROVE_OPTION) {
					...
					// 删除确认框，用于确定用户是否确定删除
					int x = JOptionPane.showConfirmDialog(panel, "Confirm delete?", "Please confirm",
					...
					if (file.exists()) {
						...
						if (x == JOptionPane.OK_OPTION) {
							...
							// 打印删除成功提示信息
							JOptionPane.showMessageDialog(panel, "Delete Success!", "information",
